...
@endif
    ...
    <div id="pf7" class="pf w0 h0" data-page-no="54">
        <div class="pc pc6 w0 h0 opened">
            ...
            <?php $array_1 = array(1, 2, 3); ?>

            <div class="t m0 x51 h14 y1c8 ff1 fsc fc7 sc0 ls0 ws0" style="left: 35px; bottom: 910px">
                {{ $summary->{2} ?? null }}<span class="_ _21" style="width: 140px"></span>{{ $summary->{5} ?? null }}<span class="_ _24" style="width: 120px"> </span>{{ $summary->{8} ?? null }}<span class="_ _27" style="@if(count($array_1) <= 3) @else width: 1420px @else width: 1400px; @endif"> </span><span style="@if(count($array_1) <= 3) bottom: 270px; font-size: 48px @elseif(count($array_1) > 3 && count($array_1) <= 4) bottom: 270px; font-size: 41px @else bottom: 270px; font-size: 46px  @endif">{{ implode(', ', array_slice($array_1, 0, 5)) }}</span>@if(count($array_1) > 4) <span style="bottom: 290px; font-size: 41px; display: flex; left: 2175px">{{ implode(', ', array_slice($array_1, 5)) }}</span> @endif
            </div>
